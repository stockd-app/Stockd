...
async def add_update_pantry_items(
    ...
    try:
        ...
        for item in request_data.items:
            ...
            quantity_value = (
                item.quantity_value if item.quantity_value not in [None, ""] else 0
            )
...
async def get_collaborative_recommendations(user_id: int, top_n: int = 5):
    ...
    if not all_user_ids:
        raise HTTPException(
            status_code=400, detail="No users found for collaborative filtering."
        )
    ...
    async with httpx.AsyncClient() as client:
        try:
            resp = await client.post(
                f"{AI_SERVER_URL_RECIPE_RECOMMENDER}/recommend", json=payload
            )
            resp.raise_for_status()
        except httpx.HTTPError as e:
            raise HTTPException(
                status_code=500, detail=f"AI server request failed: {str(e)}"
            )
